<%@ page import="com.enterprise.entity.*" %>
...
<%
    ...
    List<CaseType> caseTypes = SystemManage.getInstance().getCaseType();

%>
<form action="<%=path %>/manage/case" name="form" id="form" method="post">
    <div style="height:auto!important;height:550px;min-height:550px;">
        <h3 style="border-bottom: 1px solid #D7D7D7;color: #666666;font-size: 28px;padding-bottom: 20px;margin-bottom: 30px;">
            <%=getServletInfo()%>
            <a href="<%=path %>/manage/case/selectList" class="btn btn-primary"
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="10" class="tableBasic">
            ...
            <tr>
                <th style="text-align: right;" width="200">案例标题</th>
                <td style="text-align: left;"><input type="text" size="40" value="${e.title}" name="title"
                                                     data-rule="案例标题:required;title;length[1~45];"
                                                     id="title"/></td>
            </tr>
            <tr>
                <th style="text-align: right;">案例分类</th>
                <td style="text-align: left;">
                    <select name="typeid">
                        <c:forEach var="entry" items="<%=caseTypes %>">
                            <option value="${entry.id}" <c:if test="${e.typeid eq entry.id}">selected="selected" </c:if>>${entry.name}</option>
                        </c:forEach>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <th style="text-align: right;">案例内容</th>
                <td style="text-align: left;">
                    <textarea name="content" id="content" data-rule="新闻内容:required;"
